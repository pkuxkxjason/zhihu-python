...
class User(Zhihu):
    ...
    def get_data_id(self):
        ...
        data_id = 0
        try:
            data_id = self.soup.find("button", class_="zg-btn zg-btn-follow zm-rich-follow-btn")['data-id']
        except Exception, e:
            Logging.error("从User页面解析data_id失败,请检查页面是否变化.")
            pass
        return data_id
    ...
    def get_gender(self):
        ...
        """
        try:
            gender = str(self.soup.find("span",class_="item gender").i)
    ...
    def get_followees_num(self):
        followees_num = int(self.soup.find("div", class_="zm-profile-side-following zg-clear") \
    ...
    def get_location(self):
        if self.user_url == None:
            ...
            if self.soup == None:
                self.parser()
            soup = self.soup
            return soup.find("span", class_="location item")["title"]
